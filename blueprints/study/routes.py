...
def submit_quiz():
    ...
    try:
        ...
        for q in questions['questions']:
            ...
            mastery_level = 100.0 if status == "mastered" else 50.0
            
            if existing:
                cursor.execute(
                    "UPDATE progress SET mastery_level = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (mastery_level, existing['id'])
                )
            else:
                cursor.execute(
                    "INSERT INTO progress (user_id, syllabus_id, topic, mastery_level) VALUES (?, ?, ?, ?)",
                    (user_id, syllabus_id, topic, mastery_level)
                )
