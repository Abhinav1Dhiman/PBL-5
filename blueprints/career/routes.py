...
#     """
#     Extracts JSON object from AI response, handling markdown fences and mixed content.
#     """
#     try:
...
#         # Look for JSON that contains our expected keys
#         json_pattern = r'\{\s*"(?:quality_score|content_score|job_fit_score|match_percentage|strengths|gaps|suggestions)"\s*:\s*.*?\}'
...
#                     json_string = cleaned_response[start_pos:i+1]
#                     try:
#                         json.loads(json_string)
#                         return json_string
#                     except json.JSONDecodeError:
...
#         logger_func(f"No valid JSON object found in Groq response: {cleaned_response[:300]}")
#         return None
#     except Exception as e:
#         logger_func(f"Error extracting JSON from response: {str(e)}")
#         return None
...
# @career_bp.route('/plan', methods=['GET'])
# def plan_page():
...
def analyze_resume():
    ...
    if resume_file and resume_file.filename:
        ...
        if success:
            ...
            try:
                ...
                # --- FIX START: Removed the overly broad 'if "Error" in extracted_text' check.
                # The word "Error" in the resume content (e.g., "Error Analysis") was triggering a false positive.
                if extracted_text:
                    resume_text = sanitize_text(extracted_text)
                # --- FIX END ---
    ...
    # --- START DEBUGGING LOG ---
    log_info(f"Validation Check: Resume Text Length: {len(resume_text) if resume_text else 0}, Job Desc Length: {len(job_description) if job_description else 0}")
    # --- END DEBUGGING LOG ---
